...
function SoulBadgeQuest:isDone()
	if (hasItem("Soul Badge") and hasItem("HM03 - Surf") and getMapName() == "Route 15") or getMapName() == "Vermilion City" or getMapName() == "Route 20"then
		return true
	else
		return false
	end
...
function SoulBadgeQuest:canEnterSafari()
	return getMoney() > 5000 and not isNight() 
end
...
function SoulBadgeQuest:SafariStop()
	if self:needPokemart_() then
		self:pokemart_()
	elseif not hasItem("HM03 - Surf") and self:canEnterSafari() and game.isTeamFullyHealed() then
			return talkToNpcOnCell(7,4)
	elseif not game.isTeamFullyHealed() or not dialogs.questSurfAccept.state then
			return moveToMap("Fuchsia City")
	else
		return moveToMap("Fuchsia City")
	end
...
function SoulBadgeQuest:Route15StopHouse()
	...
	elseif self:needPokecenter() or not self.registeredPokecenter == "Pokecenter Fuchsia" then
		return moveToMap("Fuchsia City")
...
function SoulBadgeQuest:PokecenterVulcanicTown()
return moveToMap("Vulcanic Town")
end
function SoulBadgeQuest:VulcanicTown()
return moveToMap("Vulcan Forest")
end
function SoulBadgeQuest:VulcanForest()
return moveToMap("Vulcan Path")
end
function SoulBadgeQuest:VulcanPath()
return moveToMap("Vulcan Island shore")
end
function SoulBadgeQuest:VulcanIslandshore()
return talkToNpcOnCell(11,47)
end
